...
const renderRow = (assignment: Assignment) => {
  const teacherFile = (assignment as any).attachments?.teacherFiles?.[0];
  return (
    <tr key={assignment._id} className="text-left text-gray-500 text-sm even:bg-slate-50 hover:bg-purple-100">
      ...
      <td className="py-3">
        {teacherFile?.fileUrl ? (
          <a href={teacherFile.fileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
            {teacherFile.fileName || "View Attachment"}
          </a>
        ) : (
          "No Attachment"
        )}
      ...
      <td className="py-3">
        <a href={`/dashboard/list/assignment/${assignment._id}`} className="text-blue-500 hover:underline">
          View Details
        </a>
      </td>
    </tr>
    ...
    <div className="bg-white rounded-md flex-1 m-4 mt-0 p-1 sm:p-2 md:p-3 lg:p-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-4">
        <h1 className="text-md sm:text-md md:text-lg lg:text-xl 2xl:text-xl font-semibold mb-6">
          {studentInfo ? `${studentInfo.name} ${studentInfo.surname} Assignment Details` : 'Your Children Assignment Details'}
        </h1>
